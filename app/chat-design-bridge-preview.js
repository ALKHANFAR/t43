(()=>{'use strict';
 ...
 window.SIYADAH_DESIGN_RESPONSE=async(body,data)=>{
  if(body.op!=='hydrate')return data;
  try{const d=await send({op:'design_list'});return {...data,pending_work:[...(data.pending_work||[]),...(d.pending_work||[])]};}catch{return {...data,design_restore_unverified:true};}
 };
