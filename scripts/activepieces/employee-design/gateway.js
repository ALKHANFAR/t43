...
function companyContext({snapshot,ownerId,goal,userFacts=[],maxChars=12000}){
 ...
 for(const doc of snapshot.documents)for(const [index,c] of doc.chunks.entries()){
  ...
  if(score)candidates.push({score,doc,index,c,matched});
 }
 candidates.sort((a,b)=>b.score-a.score||a.doc.id.localeCompare(b.doc.id)||a.index-b.index);
 ...
 const result={ownerId,goal,companyProfile:snapshot.profile,userConfirmedFacts:facts,websiteEvidence:[],coverage:snapshot.coverage,retrieval:{mode:'multilingual_lexical',matchedChunks:candidates.length,includedChunks:0,omittedChunks:0},policy};
...
const expand=s=>({...s,documents:buildSnapshot({ownerId:s.ownerId,website:s.website,generation:s.generation,observedAt:s.observedAt,pages:s.documents.map(d=>({markdown:d.markdown,metadata:{sourceURL:d.url,title:d.title,description:d.description,statusCode:200}})),coverage:s.coverage}).documents});
...
async function exportCustomer455({api,state,session}) {
 ...
 for(const employee of employees){
  const data=employee.data||{};if(!data.flowId)fail('employee_flow_missing');
...
async function gateway455(inputs) {
 ...
 if(op==='design_start'){
  if(typeof body.goal!=='string'||body.goal.trim().length<8||body.goal.length>3000||!/^[A-Za-z0-9_-]{8,80}$/.test(body.request_id||''))return respond({ok:false,error:'goal_invalid'},400);
 ...
 if(op==='company_knowledge_context')return respond({ok:true,context:await siteKnowledge455.nativeCompanyContext455({state,ownerId:session.companyId,goal:body.goal})});
  ...
  if(op==='hydrate'){
   const team=[];for(const e of employees){const f=await api.getFlow({flowId:e.flowId});team.push({...e,status:f.status==='ENABLED'?'active':'disabled',flow_status_verified:f.status==='ENABLED'&&!!f.publishedVersionId});}
