...
(function(){
  ...
  function pinHtml(e){
    ...
    return '<div class="pin"><div class="pin__r1"><span class="av">'+esc(e.ini)+'</span><div class="pin__t"><p class="pin__n">'+esc(e.n)+' <span>· '+esc(e.r)+'</span> <button type="button" class="pinbtn tip" id="renameBtn" data-tip="إعادة تسمية" aria-label="إعادة تسمية '+esc(e.n)+'"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M4 20l4-1L18 7l-3-3L5 15l-1 5z"/><path d="M13 6l3 3"/></svg></button></p><div class="pin__s">'+esc(e.since)+'</div>'+siyRefsHtml([['الموظف',e.id],['الفلو',e.flowId]])+'</div>'+
      ...
      '<span class="swl" style="font-size:.8rem;color:var(--ash)"><span id="onLbl">'+(window.__SIY_REAL__?(siyEmployeeStatePending[e.id]?'جارٍ التحقق من تغيير الحالة…':(e.contractVersion===2?(e.setupStatus==='awaiting_connections'?'بانتظار الربط':'يحتاج إعدادًا'):(e.on?'نشط في السجل':'متوقف في السجل'))):(e.on?(f?'شغّالة':'شغّال'):(f?'متوقفة':'متوقف')))+'</span><button type="button" class="sw" id="onSw" role="switch"'+(e.contractVersion===2?' disabled':siyEmployeeStatePending[e.id]?' disabled aria-busy="true"':'')+' aria-checked="'+e.on+'" aria-label="تشغيل '+esc(e.n)+'"></button></span></div></div>'+
  ...
  function siyContractHtml455(e){
    if(e.contractVersion!==2||!e.contract)return '';
    var c=e.contract;
    var h='<div class="card__h"><b>خطة الموظف وأسباب اختيار الأدوات</b></div><div class="card__b"><ol>';
    (c.steps||[]).forEach(function(s){h+='<li><b>'+esc(TN[s.pieceName.replace(/^@activepieces\/(?:piece-)?/,'')]||s.pieceName.replace('@activepieces/piece-',''))+'</b> — '+esc(s.reason)+'</li>';});
    h+='</ol><b>متطلبات الإعداد والربط</b><ul>';
    (c.bindings||[]).forEach(function(b){h+='<li>'+esc(b.label||b.property||b.id||'حساب مطلوب')+'</li>';});
    (c.missing||[]).concat(c.issues||[]).forEach(function(m){h+='<li>'+esc(typeof m==='string'?m:m.reason||m.detail||'يلزم التحقق من إحدى الخطوات')+'</li>';});
    h+='</ul><b>كيف نتحقق من النتيجة</b><ul>';
    (c.evidence||[]).forEach(function(v){h+='<li>'+esc(v.metric)+' — '+esc(v.check)+'</li>';});
    return h+'</ul><p>لم يبدأ التنفيذ الخارجي. يلزم استكمال الإعداد والربط واختبار التشغيل.</p></div>';
  }
  function instrHtml(e){
    if(window.__SIY_REAL__){
      ...
      return '<div class="card"><div class="card__h">'+I.pen+'<b>تعليماته</b><span class="cnt">المحفوظة في السجل</span></div>'+
        ...
        '<p>هذه إعدادات محفوظة؛ تطبيق الاستئذان والقيود أثناء التشغيل لم يُتحقق منه هنا.</p></div>'+siyContractHtml455(e)+'</div>';
    }
  ...
  async function siyRequest(body){
    if(window.SIYADAH_DESIGN_REQUEST){var routed=await window.SIYADAH_DESIGN_REQUEST(body);if(routed!==undefined)return routed;}
  ...
  async function siySetEmployeeState(e){
    if(e?.contractVersion===2){window.alert('راجع متطلبات الإعداد والربط في تعليمات الموظف قبل تشغيله.');return;}
  ...
  function mapEmployee(m,i){
    ...
    return { id:m.recordId, contractVersion:m.contractVersion||1, contract:m.contract||null, setupStatus:m.status, flowId:m.flowId||null, n:displayName, r:role, ini:String(m.initial||displayName.slice(0,1)),
      f:i%2===1, on:/^(نشط|active)$/i.test(String(m.status||"").trim()), wait:0, waits:[],
      since:(m.contractVersion===2?(m.status==='awaiting_connections'?'بانتظار ربط الحسابات والتحقق من التشغيل':'يحتاج استكمال الإعداد'):/^(نشط|active)$/i.test(String(m.status||"").trim())?"مسجل كنشط — التشغيل لم يُتحقق منه":"مسجل كمتوقف"), ver:1,
  ...
  function siyBoot(){
    ...
    // Carry a reviewed onboarding request as an unsent draft, never an automatic message.
    try {
      var onboardingDraft=JSON.parse(sessionStorage.getItem('siyadah_onboarding_draft')||'null');
      var composer=document.getElementById('input');
      if(onboardingDraft&&typeof onboardingDraft.text==='string'&&Date.now()-onboardingDraft.at<3600000&&composer&&!composer.value){
        composer.value=onboardingDraft.text;composer.dispatchEvent(new Event('input',{bubbles:true}));
        sessionStorage.removeItem('siyadah_onboarding_draft');
      }
    } catch(e) {}
