...
async function workerFinish455(inputs){
 ...
 try{
  ...
  if(intent.intent==='CREATE_EMPLOYEE'){return dispatchUnifiedFactory455({state,work,ownerId:inputs.owner,goal:intent.goal,signingSecret:inputs.signingSecret,sign:(value,key)=>require('crypto').createHmac('sha256',key).update(value).digest('hex')});}
